...
def predict_from_classifier (base_dir):
...
#
 ...
 # 
  ...
  out_file.iloc[0]=out_file.iloc[0].map(lambda x: x.replace(rm_str, ''))
  #out_file = out_file.replace({rm_str:''})
  out_file.to_csv(out_dir, index=False, header=False)
  

      
#####

def removing_files (directory, class_res):
  import glob, os
  import csv
  import pandas as pd
  import argparse
