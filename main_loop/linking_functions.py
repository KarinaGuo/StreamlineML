def predict_leaf():
  ...
  parser.add_argument("base_dir", help="Parent directory where data and folders are found", type=str,)
  parser.add_argument("loop", help="Which loop is it in", type=str)
  ...
  if args.loop == model:  
    ...
    model_predictions_file = os.path.join(out_dir, "_predictions.npy")
    thresh_iou = 0.7
    matches_out_file= os.path.join(out_dir, "model_matches.csv")
    ...
    instances_out_file= os.path.join(out_dir, "model_summary.csv")
...
####
def CCA() # Creates a mask of the largest component
...
def extract_leaves():
  ...
  parser.add_argument("base_dir", help="Parent directory where data and folders are found", type=str,)
  parser.add_argument("loop", help="Which loop is it in", type=str,)
  parser.add_argument("CCA", help="If run CCA", type=str, default = "Y")
  args = parser.parse_args()
  if args.loop == "main":
...
def predict_from_classifier ():
  ...
  if args.loop == "main":
    ...
    #### Need to alter the output to cleanup filename
    #out_dir_temp = os.path.join(args.base_dir, "temp_classifier_results_test.csv")
    #rm_str = os.path.join(data_dir, "subf/")
    #out_file = pd.read_csv(out_dir_temp)
    #out_file = out_file.replace({rm_str:''})
    results.to_csv(out_dir, index=False, header=False)
...
def cut_focal_box (base_dir)
	...
	def cut_label_files(fileNameNoExtension, jsonPath, imagePath, cutDir, focalbox, use_categories):
		...
		if __name__ == "__main__":
			...
			parser = argparse.ArgumentParser(description="")
      parser.add_argument("base_dir", help="Parent directory where data and folders are found", type=str,)
			label_directory = os.path.join (base_dir, "input_d2_data/")
			new_directory = os.path.join (base_dir, "trim_d2_data/")
			...
			for num, json_file in enumerate(labelme_json):
				...
				cut_label_files(fileNameNoExtension, json_file, jpg_file, new_directory, args.focalbox, use_categories)
...
def lm2coco ()
    ...
    parser.add_argument("--output", help="Output json file path.", default="trainval.json")
		parser.add_argument("--polyORbb", help="poly OR bb", default="poly")
		parser.add_argument("--classes", action="append", nargs="+", type=str)
		args = parser.parse_args()
    labelme_images = os.path.join(base_dir, "trim_d2_data/")   
		import itertools 
		arg_classes = list(itertools.chain(*args.classes))
...
def train_leaf (base_dir)
	...
	# this required a highly prescribed directory and file structure, with a directory called training_name containing images and .json files, and a file called training_name+'.json'
	parser = argparse.ArgumentParser(description="labelme annotation to coco data json file.")
    parser.add_argument("base_dir", help="Parent directory where data and folders are found", type=str)
    args = parser.parse_args()
...
def train_classifier(base_dir)
	...
	data_dir   = os.path.join(args.base_dir, "input_classifier_data")
	val_ratio  = 0.2
	num_epochs = 42 
	model_out  = os.path.join(args.base_dir, "model/classifier/model.pth")
